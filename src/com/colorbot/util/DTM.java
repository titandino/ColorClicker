...
public class DTM {
	 ...
	 */
	public boolean findOriginalDTM(BufferedImage image, final int tol) {
		return checkAllPoints(image, loc[0], tol);
	}
	 ...
	 * @return <tt>true</tt> if a DTM was found; otherwise <tt>false</tt>
	 */
	public boolean findDTMS(BufferedImage image, final int tol) {
		return findDTMS(image, tol, new Rectangle(0, 0, Bot.SCREEN_WIDTH, Bot.SCREEN_HEIGHT));
	}
	 ...
	 * @return <tt>true</tt> if a DTM was found; otherwise <tt>false</tt>
	 */
	public boolean findDTMS(BufferedImage image, final int tol, final Rectangle rec) {
		boolean foundDTM = false;
		dtmLocations.clear();
		for (int y = rec.y; y < rec.y + rec.height; y++) {
			for (int x = rec.x; x < rec.x + rec.width; x++) {
				if (checkColor(Bot.getColorAt(image, x, y), rgb[0], tol)) {
					final Rectangle curbox = new Rectangle(x - (loc[0].x - box.x), y - (loc[0].y - box.y), box.width, box.height);
					if (checkAllPoints(image, new Point(x, y), tol)) {
	...
	public Point getDTM(final int i) {
		return dtmLocations.isEmpty() ? null : new Point(dtmLocations.get(i).x + (dtmLocations.get(i).width / 2), dtmLocations.get(i).y + (dtmLocations.get(i).height / 2));
	}
	 ...
	 */
	private boolean checkAllPoints(BufferedImage image, Point locM, final int tol) {
		try {
			...
			for (int i = 0; i < loc.length; i++) {
				...
				final Color colorToCheck = Bot.getColorAt(image, pointToCheck);
	...
	private boolean checkColor(final Color c, final Color c2, final int tol) {
		return (checkColor(c.getRed(), c2.getRed(), tol) && checkColor(c.getGreen(), c2.getGreen(), tol) && checkColor(c.getBlue(), c2.getBlue(), tol));
	}
